...
def main() -> None:
    ...
    try:
        ...
        print(f"Commit SHA found {commit_sha}")
        ...
        success = True
        print(f"Successfully created zip for {args.integration} version {args.version}")
        ...
        if success:
            try:
                ...
                print(
                    f"Couldn't fully restore environment, but zip was created successfully: {e}"
                )
                print(
...
def _validate_integration(integration: str) -> None:
    ...
    if not integration_path.exists():
        raise ValueError(
            f"the value of -i/--integration wasn't found in {integration_path}"
        )


def _validate_dir(dir_) -> pathlib.Path:
    zip_dir: pathlib.Path = pathlib.Path(dir_) if dir_ else MARKETPLACE_PATH
    if not zip_dir.exists():
        raise FileNotFoundError(f"Could not find dir {zip_dir}")
...
def get_commit_sha(integration: str, version: float) -> str:
    ...
    try:
        ...
        result = subprocess.run(
            cmd, cwd=MARKETPLACE_PATH, shell=True, capture_output=True, text=True
        )
        ...
        print(f"Error searching in origin/develop branch, path {integration_def}: {e}")
    raise ValueError(
