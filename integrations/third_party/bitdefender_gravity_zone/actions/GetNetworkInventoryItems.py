...
def main():
    ...
    filter_groups = siemplify.extract_action_param(
        "Filter - Groups",
        print_value=True,
        default_value=False,
        input_type=bool,
    )
    filter_ec2 = siemplify.extract_action_param(
        "Filter - EC2 Instances",
        print_value=True,
        default_value=False,
        input_type=bool,
    )
    filter_computers = siemplify.extract_action_param(
        "Filter - Computers",
        print_value=True,
        default_value=False,
        input_type=bool,
    )
    filter_vms = siemplify.extract_action_param(
    ...
    try:
        ...
        inventory_items_data = mtm.network_inventory_list(
            access_url,
            parent_id,
            filter_groups,
            filter_ec2,
