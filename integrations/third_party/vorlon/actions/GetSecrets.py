...
def main():
    ...
    siemplify.script_name = GET_SECRETS_SCRIPT_NAME
    ...
    try:
        ...
        secrets = manager.get_secrets(secrets=secret_ids, service=service_id)
        siemplify.result.add_result_json(secrets)
        result = True
        status = EXECUTION_STATE_COMPLETED
        output_message = f"Successfully fetched secret details from the {INTEGRATION_DISPLAY_NAME} server"

    except Exception as e:
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Failed to fetch secret details from {INTEGRATION_DISPLAY_NAME} server! {e}"
