...
from ..core.constants import GET_IP_INFO_SCRIPT_NAME, INTEGRATION_NAME
from ..core.IPInfo import IPInfo
...
@output_handler
def main():
    """Get IP Details action is used to fetch the IP information for the given IP Address from Team Cymru Scout."""
    ...
    try:
        ...
        output_message = (
            additional_msg
            + f"Successfully fetched the IP Information for the following {is_success} out of {len(valid_ips)} valid IPs: {successful_ips} from {INTEGRATION_NAME}.\n\n"
        )
        ...
        siemplify.LOGGER.error(
            f"General error performing action {GET_IP_INFO_SCRIPT_NAME}.",
        )
        siemplify.LOGGER.exception(response)

        status = EXECUTION_STATE_FAILED
        output_message = f"Failed to fetch IP information\nError: {response}"
