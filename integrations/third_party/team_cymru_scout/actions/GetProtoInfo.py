...
from ..core.constants import GET_PROTO_INFO_SCRIPT_NAME, INTEGRATION_NAME
...
@output_handler
def main():
    """Get Proto Info action is used to fetch the Proto information for the given IP Address from Team Cymru Scout."""
    ...
    try:
        ip_info = IPInfoSection(siemplify, section="proto_by_ip")
        ...
        output_message = (
            additional_msg
            + f"Successfully fetched the Proto Information for the following {is_success} out of {len(valid_ips)} valid IPs: {successful_ips} from {INTEGRATION_NAME}.\n\n"
        )
        ...
        siemplify.LOGGER.error(
            f"General error performing action {GET_PROTO_INFO_SCRIPT_NAME}.",
        )
        siemplify.LOGGER.exception(response)

        status = EXECUTION_STATE_FAILED
        output_message = f"Failed to fetch Proto information\nError: {response}"
