...
def main():
    ...
    try:
        revoked_security_group = ec2_manager.revoke_security_group_ingress(
            group_id,
            ip_permisssions_dict,
        )
        siemplify.LOGGER.info(
            f"The inbound ip permissions of the security group id {group_id} were revoked successfully",
        )
        ...
        siemplify.LOGGER.error(
            f"Error occured when revoking the inbound ip permissions of the secrurity group id {group_id}.\nError: {e}",
        )
