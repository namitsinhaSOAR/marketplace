...
SCRIPT_NAME = "Show Allowed IP Lists"
...
def main():
    ...
    results = bandura_cyber_manager.show_allowed_ip_list(list_name=list_name)
    ...
    if results:
        ...
        siemplify.result.add_attachment(
            "Bandura Allowed IP Lists Output",
            "allowed_ip_lists.txt",
            json.dumps(results),
        )

        # Add data table
        siemplify.result.add_data_table(
            "Bandura Allowed IP Lists",
            construct_csv(results),
        )

        siemplify.result.add_result_json(results)
        output_message = "Following Allowed IP Lists were found.\n"
        result_value = True
    else:
        output_message = "No Allowed IP Lists were found."
        result_value = False
