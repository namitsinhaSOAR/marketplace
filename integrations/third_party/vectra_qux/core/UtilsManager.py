...
class HandleExceptions:
    ...
    def get_handler(self):
        ...
        return {
            ...
            ADD_OUTCOME_API_NAME: self.add_outcome_action,
            ...
            DESCRIBE_ENTITY_API_NAME: self._handle_not_found_error,
            DESCRIBE_DETECTION_API_NAME: self._handle_not_found_error,
            DOWANLOAD_PCAP_API_NAME: self.get_file,
    ...
    def _handle_general_error(self):
        ...
        return VectraQUXException, error_msg

    def _handle_bad_request_error(self):
        """Handles bad request errors by extracting the appropriate error message from the response.
        ...
        if isinstance(error_response, dict):
            # If the response is a dict, extract the error message
    ...
    def get_tags(self):
        """Handler for tag related action.
    ...
    def add_outcome_action(self):
        """The default handler for add outcome action.
...
def extract_fields(response, mandatory_fields):
    """Extracts fields from a response dictionary based on a given set of mandatory fields
    If the number of fields in the response is less than the number of mandatory fields,
    the entire response is returned. Otherwise, the mandatory fields are extracted in order
    and the remaining fields are filled in up to the number of mandatory fields.

    :param response: The response dictionary
    :param mandatory_fields: The list of mandatory fields to extract
...
def validate_triage(triage_as):
    ...
    Raises:
        VectraQUXException: If it contains invalid characters or length less than 4.
    ...
    if any(char in triage_as for char in invalid_characters):
        raise VectraQUXException("Error: triage as contains invalid characters.")

    if len(triage_as) < 4:
        raise VectraQUXException("Error: triage as should be more than 3 characters.")
...
def get_alert_id(entity_id, last_timestamp, entity_type):
    return f"{entity_type}#{entity_id}-{last_timestamp}"
...
def save_timestamp_for_job(
    ...
    try:
        ...
        raise VectraQUXException(f"Failed saving timestamps to db, ERROR: {e}")
...
def fetch_timestamp_for_job(
    ...
    try:
        ...
        raise VectraQUXException(f"Failed reading timestamps from db, ERROR: {e}")
...
def process_action_parameter_integer(action_parameter, field_name):
    """Processes the given action parameter by splitting it by comma, stripping
    each of the resulting strings and returning them as a list of integer.
