...
def main():
    ...
    try:
        validate_integer(entity_id, field_name="Entity ID")
        validate_integer(user_id, field_name="User ID")
        ...
        # get assignment_id
        assignment_info = received_info.assignment
        if not assignment_info:
            ...
            # update assignment with the given user_id
            response, assignments = vectra_manager.update_assignment(
                user_id,
                assignment_id,
            )
            output_message = (
            ...
            siemplify.result.add_data_table(
                ...
                data_table=construct_csv(
                    [assignments.update_assignment_csv(entity_type)],
                ),
            )

    except (UserNotPermittedException, InvalidIntegerException) as e:
