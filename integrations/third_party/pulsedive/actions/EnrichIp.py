    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                is_risky = False
                ip_data = manager.get_indicator_data(
                    indicator=identifier,
                    retrieve_comments=retrieve_comments,
                    comment_limit=max_returned_comments,
                )

                if int(RISK_SCORE.get(ip_data.threshold)) >= int(
                ...
                json_results[get_entity_original_identifier(entity)] = ip_data.to_json(
                    comments=ip_data.comments,
                )

                # Create case wall table for comments
                if ip_data.comments:
                    comments_table = construct_csv(
                        [comment.to_table() for comment in ip_data.comments],
                    )
        ...
        if successful_entities:
            output_message += f"Successfully enriched the following IPs using {PROVIDER_NAME}: \n {', '.join([get_entity_original_identifier(entity) for entity in successful_entities])} \n"
            siemplify.update_entities(successful_entities)

        if failed_entities:
            output_message += f"Action wasn’t able to enrich the following IPs using {PROVIDER_NAME}: \n {', '.join(failed_entities)} \n"
        ...
        output_message = f"Error executing action “Enrich IP”. Reason: {err}"
        result_value = False
