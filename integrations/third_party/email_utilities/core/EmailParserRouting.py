...
from . import EmailParserDecode, EmailParserRegex
...
def noparenthesis(line: str) -> str:
    ...
    while True:
        lline = line_
        line_ = EmailParserRegex.noparenthesis_regex.sub("", line_)
        if lline == line_:
            break
...
def cleanline(line: str) -> str:
    ...
    return EmailParserRegex.cleanline_regex.sub("", line)
...
def parserouting(line: str) -> dict[str, typing.Any]:
    ...
    npline = line.replace(
        ")",
        " ) ",
    )  # normalise space # Re-space () ")by " exists often
    ...
    npline = npline.strip("\n")  # Remove any new-line
    raw_find_data = EmailParserRegex.date_regex.findall(
        npline,
    )  # extract date on end line.
    ...
    if out.get("for"):
        ...
        m = EmailParserRegex.email_regex.findall(out["for"])
        if m:
            out["for"] = list(set(m))
        else:
            del out["for"]
